...
class Parser {
  ...
  /**
   * parsed SQL to basic mongoose object
   * @output {
     table1: {
       field1: {
         type: 'integer',
   ...
   * 
   */
  toMongooseJsonObject() {
    return this.toJsonSchemaArray()
      .reduce((object, item) => {

        object[item.title] = Object.keys(item.definitions).reduce((obj, key) => {

          const value = item.definitions[key];
          value.required = item.required.includes(key);

          value.min = value.minimum || value.minLength;
          value.max = value.maximum || value.maxLength;
