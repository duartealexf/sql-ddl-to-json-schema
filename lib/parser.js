...
class Parser {
  ...
  toJsonSchemaArray(tables = null) {
    if (!tables) {
      tables = this.toCompactJson(tables);
    }
   ...
   * @param {number} indent Number of spaces to indent JSON output in files.
   * @returns {Promise<string[]>} Resolved promise with output file paths.
   */
  toJsonSchemaFiles(outputDir, jsonSchemas = null, indent = 2) {
    if (!outputDir) {
    ...
    return Promise.all(
      jsonSchemas.map(schema => {
        return new Promise(resolve => {
          ...
          fs.writeFile(filepath, JSON.stringify(schema, null, indent), err => {
            if (err) {
