...
import { O_CREATE_TABLE_CREATE_DEFINITION, P_CREATE_INDEX, STATEMENT, O_CREATE_TABLE_CREATE_DEFINITION_FULLTEXT_INDEX } from '@mysql/compiled/typings';

/**
 * Fulltext index of a table.
 */
export class FulltextIndex implements FulltextIndexInterface, ClonableInterface, SerializableInterface{
  name?: string;
  columns:IndexColumn[] = [];
  ...
  static fromDef(json: O_CREATE_TABLE_CREATE_DEFINITION | P_CREATE_INDEX): FulltextIndex {
    if (json.id === 'O_CREATE_TABLE_CREATE_DEFINITION') {
      return FulltextIndex.fromObject(json.def.fulltextIndex as O_CREATE_TABLE_CREATE_DEFINITION_FULLTEXT_INDEX);
    }
   ...
   */
  static fromObject(json: O_CREATE_TABLE_CREATE_DEFINITION_FULLTEXT_INDEX | P_CREATE_INDEX["def"]): FulltextIndex {
  ...
  clone(): FulltextIndex {
    ...
    if (isDefined(this.name)) { index.name = this.name; }
    if (isDefined(this.options)) { index.options = this.options.toJSON(); }
