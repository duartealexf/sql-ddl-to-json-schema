...
class Parser {
  ...
  get results() {
    ...
    try {
       ...
       */
      if (e.message && utils.isString(e.message) && /^invalid syntax at line \d+/.test(e.message)) {
        e.message = e.message.replace(/\d+/, lineCount);
      }
      throw e;
    }

    /**
     * Reset remains to not affect line count of next feed.
     */
    this.remains = '';
