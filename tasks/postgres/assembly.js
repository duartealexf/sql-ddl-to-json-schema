 ...
 * folder concatenating them all to lib/postgres/grammar.ne, so it can be
 * later compiled to grammar.js by nearleyc.
 *
 * It is also responsible for copying the dictionary and shared folder to
 * lib/postgres folder (so it can be read by the compiled grammar files).
 */
 ...
 */
const keywords = require('../../src/postgres/parser/dictionary/keywords');
 ...
 * Input.
 */
const srcFolder = path.join(projectRoot, 'src', 'postgres', 'parser');
 ...
 * Output.
 */
const compiledFolder = path.join(projectRoot, 'lib', 'postgres', 'parser');
